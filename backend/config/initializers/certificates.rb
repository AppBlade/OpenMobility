

if Rails.env.test?

  ScepKey = OpenSSL::PKey::RSA.new 2048
  ApnsKey = OpenSSL::PKey::RSA.new 2048
  AppleiPhoneDeviceKey = OpenSSL::PKey::RSA.new 2048

  ScepCert = OpenSSL::X509::Certificate.new.tap do |cert|
    cert.version = 1
    cert.serial = 1
    cert.not_before = Time.now
    cert.not_after = Time.now + 10.year
    cert.public_key = ScepKey.public_key
    cert.subject = OpenSSL::X509::Name.parse "/C=US/ST=MA/L=Boston/O=AppBlade/"
    cert.issuer = cert.subject
    cert.sign ScepKey, OpenSSL::Digest::SHA1.new
  end

  ApnsCert = OpenSSL::X509::Certificate.new.tap do |cert|
    cert.version = 1
    cert.serial = 1
    cert.not_before = Time.now
    cert.not_after = Time.now + 1.year
    cert.public_key = ApnsKey.public_key
    uuid = SecureRandom.uuid
    cert.subject = OpenSSL::X509::Name.parse "/UID=com.apple.mgmt.External.#{uuid}/CN=APSP:#{uuid}/C=US"
    cert.issuer = cert.subject
    cert.sign ApnsKey, OpenSSL::Digest::SHA1.new
  end

  AppleIPhoneDeviceCA = OpenSSL::X509::Certificate.new.tap do |cert|
    cert.version = 1
    cert.serial = 1
    cert.not_before = Time.now
    cert.not_after = Time.now + 1.year
    cert.public_key = AppleiPhoneDeviceKey.public_key
    uuid = SecureRandom.uuid
    cert.subject = OpenSSL::X509::Name.parse "/C=US/O=Apple Inc./OU=Apple iPhone/CN=Apple iPhone Device CA"
    cert.issuer = cert.subject
    cert.sign AppleiPhoneDeviceKey, OpenSSL::Digest::SHA1.new
  end

  AppleDeviceX509Store = OpenSSL::X509::Store.new
  AppleDeviceX509Store.add_cert AppleIPhoneDeviceCA

else

  scep_key_path = File.join(Rails.root, 'ssl', 'cakey.pem')
  ScepKey = OpenSSL::PKey::RSA.new File.read(scep_key_path), ENV['SCEP_PASSPHRASE']

  scep_cert_path = File.join(Rails.root, 'ssl', 'cacert.pem')
  ScepCert = OpenSSL::X509::Certificate.new File.read(scep_cert_path)

  apns_key_path = File.join(Rails.root, 'ssl', 'apnskey.pem')
  ApnsKey = OpenSSL::PKey::RSA.new File.read(apns_key_path), ENV['APNS_PASSPHRASE']

  apns_cert_path = File.join(Rails.root, 'ssl', 'apnscert.pem')
  ApnsCert = OpenSSL::X509::Certificate.new File.read(apns_cert_path)

  AppleRootCA = OpenSSL::X509::Certificate.new "-----BEGIN CERTIFICATE-----

UKqK1drk/NAJBzewdXUh
-----END CERTIFICATE-----"

  AppleIPhoneCA = OpenSSL::X509::Certificate.new "-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----"

  AppleIPhoneDeviceCA = OpenSSL::X509::Certificate.new "-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----"

  AppleDeviceX509Store = OpenSSL::X509::Store.new
  AppleDeviceX509Store.add_cert AppleRootCA
  AppleDeviceX509Store.add_cert AppleIPhoneCA
  AppleDeviceX509Store.add_cert AppleIPhoneDeviceCA

end

ApnsTopic = ApnsCert.subject.to_a.find{|(oid, value)| oid == 'UID'}[1]
